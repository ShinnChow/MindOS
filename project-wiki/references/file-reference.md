...
## 07-human-insights.md — 人机协作洞察

**为什么需要：** 现有文件全部面向"项目"和"Agent"，没有一个面向"使用 Agent 的人"自身成长。与 Agent 协作过程中积累的知识、发现的高效/低效交互模式、沉淀的 SOP、遇到的好技术——这些经验如果不记下来，换个项目又从零开始。

**与其他文件的区别：**
- `80-known-pitfalls.md`：项目级别的技术坑（面向 Agent）
- `07-human-insights.md`：个人级别的能力积累（面向你自己）

**写给谁看：** 未来的你（不写给 Agent）

**核心内容：**
- 知识学习与经验积累（通过协作学到的新知）
- 交互模式分析（哪些 prompt 模式高效/低效）
- SOP 优化与沉淀（可复用到其他项目的标准流程）
- 技术发现与推荐（Agent 引入的值得记住的技术方案）
- 诊断建议（流程瓶颈、效率问题、改进思路）
