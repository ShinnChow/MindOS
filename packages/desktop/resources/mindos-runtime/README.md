...
1. From the **monorepo root**, run a production Next build so `packages/web/.next/standalone/server.js` exists, e.g. `pnpm --filter @mindos/web build`. The app uses **`output: 'standalone'`**; `prepare-mindos-runtime` syncs `.next/static` and `public` into the bundled runtime's `packages/web` layout per Next's deployment docs.
2. Run:
