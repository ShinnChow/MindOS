...
describe('resolveScript', () => {
  ...
  it('returns MINDOS_PROJECT_ROOT/app/scripts/<name> when it exists', () => {
    process.env.MINDOS_PROJECT_ROOT = '/home/user/.mindos/runtime';
    const expected = path.join('/home/user/.mindos/runtime', 'app', 'scripts', 'extract-pdf.cjs');
    existingPaths.add(expected);

    expect(resolveScript('extract-pdf.cjs')).toBe(expected);
  });

  it('returns cwd/scripts/<name> in dev mode (cwd = app/)', () => {
    delete process.env.MINDOS_PROJECT_ROOT;
    process.cwd = () => '/project/app';
    const expected = path.join('/project/app', 'scripts', 'extract-pdf.cjs');
    existingPaths.add(expected);

    expect(resolveScript('extract-pdf.cjs')).toBe(expected);
  });

  it('falls back to cwd/../app/scripts/<name> in standalone mode', () => {
    delete process.env.MINDOS_PROJECT_ROOT;
    process.cwd = () => '/project/app/.next/standalone';
    const expected = path.resolve('/project/app/.next/standalone', '..', 'app', 'scripts', 'extract-pdf.cjs');
    existingPaths.add(expected);
  ...
  it('falls back to cwd/.next/standalone/scripts/<name>', () => {
    delete process.env.MINDOS_PROJECT_ROOT;
    process.cwd = () => '/project/app';
    const expected = path.join('/project/app', '.next', 'standalone', 'scripts', 'extract-pdf.cjs');
    existingPaths.add(expected);
  ...
  it('prefers MINDOS_PROJECT_ROOT over cwd fallbacks', () => {
    ...
    process.cwd = () => '/project/app';
    const projPath = path.join('/runtime', 'app', 'scripts', 'extract-pdf.cjs');
    const cwdPath = path.join('/project/app', 'scripts', 'extract-pdf.cjs');
  ...
  it('recovers when MINDOS_PROJECT_ROOT path is wrong but cwd/scripts/ exists (the bug scenario)', () => {
    ...
    process.cwd = () => '/Users/geminilight/.mindos/runtime/app';

    // MINDOS_PROJECT_ROOT/app/scripts/ does NOT exist (the bug)
    // but cwd/scripts/ DOES exist
    const cwdPath = path.join('/Users/geminilight/.mindos/runtime/app', 'scripts', 'extract-pdf.cjs');
    existingPaths.add(cwdPath);
