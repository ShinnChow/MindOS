...
- [x] **App 巨型文件架构重构** — 8 个 700+ 行组件/路由拆分为 22 个专职模块。TodoRenderer -85%、file/route -65%、UpdateTab/McpTab/A2aTab 各 -59%/-59%/-60%、AgentDetailContent -38%、ask/route -31%、FileTree/SyncTab -28%、AgentsSkillsSection -25%。新建 `lib/parsing/`（纯逻辑）、`lib/stores/`（hidden-files）、`lib/hooks/`（drag-drop）、子组件文件（DesktopUpdateCards/McpConnectGuides/AcpRegistrySection/AgentsSkillsByAgent/SyncEmptyState/FileTreeContextMenus 等）。测试 118 文件 1,442 tests 全过，0 新增 TS 错误。
